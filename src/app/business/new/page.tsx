...
const SAMPLES: Sample[] = [
  ...
  },
  {
    ...
    name: 'Eyesense.ai',
    description:
      "We run Eyesense.ai, a computer-vision monitoring platform for property and perimeter security; our flagship product is DeerAlarm. We connect to a customer's existing IP/RTSP cameras (Amcrest and others) through a small edge gateway on NVIDIA Jetson Orin Nano, Apple Mac Mini, or Raspberry Pi that dials OUT over a WireGuard VPN tunnel to our cloud hub at eyesense.ai — so no inbound ports are ever opened on the customer's network. Video analysis stays on-device: configurable YOLO/ONNX detection pipelines run real-time object detection (deer, wildlife, people, vehicles, smoke), with optional negative-check override via a general COCO model, sliding-window consensus gating, and SmolVLM/Ollama LLM validation before we confirm an event. Confirmed detections trigger real-time alerts by push, SMS, and email with annotated snapshots, plus automated IoT deterrents — Rachio, Orbit B-hyve, and Hydrawise sprinkler zones, LetPot hydroponics devices, Amcrest camera spotlight/siren alarms, and GPIO-controlled valves and sirens — through user-drawn Action Zones that map detections to specific devices. Users can veto false positives or override LLM rejections; those human feedback signals feed a closed-loop retraining pipeline: confirmed detections become positive YOLO labels, vetoes and LLM rejections become negatives, nightly cron (or manual jobs) balance datasets (max 5:1 neg:pos guardrails) and fine-tune Ultralytics YOLO weights on each property's own example data, export ONNX, and deploy updated models back to gateways via MinIO — so edge models improve over time without a monitoring contract. Customers can also import external datasets (Hugging Face, Roboflow) and run per-pipeline fine-tuning with adaptive hyperparameters. The edge stack is Spring Boot + React SPA + PostgreSQL + Redis + MinIO + Ollama on the gateway; the hub is a Spring Boot management service (JDBC/Liquibase) plus a reactive R2DBC Spring Cloud Gateway that proxies browser and API traffic through the VPN to each gateway, with a React SPA admin portal, EZGateway zero-touch onboarding, location-based multi-tenant access control, shareable detection links, and Stripe subscription tiers (~$15/gateway/mo with trial). Cloud infra runs on AWS ECS Fargate (ARM64), a WireGuard EC2 relay, shared RDS PostgreSQL (isolated deeralarm schema), and Valkey/Redis caching — lean at roughly $20/month per hub.",
  },
  {
    label: 'PaperIQ',
    name: 'PaperIQ',
  ...
  },
  {
    ...
    name: 'Story Lantern',
    description:
      'We run Story Lantern (story.paperiq.ai), an agentic children\'s storybook generator built as a sibling to our Agent Forge consulting stack. A parent, teacher, or creator enters a short story premise, picks a reader age band (0–2 through 9–12) and language, and optionally uploads a reference photo of a character; our story-author ToolLoopAgent (Claude via the Vercel AI SDK) drafts a synopsis and rough chapter outline, defines characters and a style bible, writes full chapter prose, and drives illustration tools to produce a consistent character sheet (the same character in multiple poses locked to the reference image) plus a scene illustration per chapter in one cohesive watercolor-style look. Images are generated server-side with Google Gemini "Nano Banana" (reference-image multi-part prompts) and optional rembg/ImageMagick cutouts for transparent character poses. Finished books open in a mobile-friendly Read mode with page turns, swipe/tap navigation, and ElevenLabs read-aloud narration with karaoke word highlighting. Stack: Next.js 14 App Router, React 18, better-sqlite3 for story + turn persistence, SSE streaming for live agent progress, Docker on our home server behind CloudFront. Audiences include parents making bedtime stories, teachers, indie authors, and grandparents who want a personalized illustrated book without hiring an illustrator.',
  },
  {
...
const NEW_BUSINESS_LEAD =
  'Describe what the business does. The consultant profiles it, writes an elevator pitch and business plan, researches the competitive landscape with live web search, drafts an advisory market assessment (sized market, demand, pros/cons, risks, and a go/no-go read — it informs you; it does not decide for you), recommends a SaaS + open-source stack (you can override later), and proposes the agent roles to forge.';
...
export default function NewBusinessPage() {
  ...
  return (
    <ForgePageShell
      ...
      nav={<ForgeTopNav variant="dashboard" back="businesses" />}
      footer={
        <footer className="ops-hud-bottom-rail">
          ...
          <p className="ops-tagline">Describe a business · forge the workforce · ship the blueprint</p>
        </footer>
      }
    >
